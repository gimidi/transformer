...
class EqualDatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        for i in pool:
            ...
            df_content = pd.read_csv(f"./data/switch/content_{i}.csv").fillna(
                {"title": "", "description": "", "text": ""}).set_index("id")
            df_topic = pd.read_csv(f"./data/switch/topics_{i}.csv").fillna({"title": "", "description": ""}).set_index(
                "id")
    ...
    def shuffle(self, missing_list=None, wrong_dict=None, max_wrong=16):
        ...
        while True:
            ...
            if len(pair_pool) > 0:
                ...
                if topic not in topic_batch and content not in content_batch and pair not in pairs_epoch and len(
                        current_batch) < self.shuffle_batch_size:
                    ...
                    if wrong_pool is not None and len(current_batch) < self.shuffle_batch_size:
                        ...
                        if wrong_pairs is not None and len(wrong_pairs) > 0:
                            ...
                            for wrong_pair in wrong_pairs[:max_wrong]:
                                ...
                                if topic_w not in topic_batch and content_w not in content_batch and wrong_pair not in pairs_epoch and len(
                                        current_batch) < self.shuffle_batch_size:
                ...
                # Oversample missing
                if missing_pool is not None and len(missing_pool) > 0 and np.random.rand() < 0.5 and len(
                        current_batch) < self.shuffle_batch_size:
        ...
        if self.typ == "train":
            ...
            for i in pool:
                ...
                if self.mode == "topic":
                    ...
                    df = pd.read_csv(f"./data/switch/topics_{i}.csv").fillna(
                        {"title": "", "description": ""}).set_index("id")
                    ...
                    df = pd.read_csv(f"./data/switch/content_{i}.csv").fillna(
                        {"title": "", "description": "", "text": ""}).set_index("id")
            ...
            if self.mode == "topic":
                ...
                df = pd.read_csv("./data/switch/content_0.csv").fillna(
                    {"title": "", "description": "", "text": ""}).set_index("id")
